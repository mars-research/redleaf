//! Linked list implementation.
/*
use core::cell::Cell;

pub struct ListLink<'a, T: 'a + ?Sized>(Option<&'a mut T>);

impl<'a, T: ?Sized> ListLink<'a, T> {
    pub const fn empty() -> ListLink<'a, T> {
        ListLink(None)
    }
}

pub trait ListNode<'a, T: ?Sized> {
    fn next(&'a mut self) -> &'a mut ListLink<'a, T>;
}

pub struct List<'a, T: 'a + ?Sized + ListNode<'a, T>> {
    head: ListLink<'a, T>,
}

//pub struct ListIterator<'a, T: 'a + ?Sized + ListNode<'a, T>> {
//    cur: Option<&'a mut T>,
//}

//impl<'a, T: ?Sized + ListNode<'a, T>> Iterator for ListIterator<'a, T> {
//    type Item = &'a mut T;
//
//    fn next(&mut self) -> Option<&'a mut T> {
//        match self.cur {
//            Some(res) => {
//                self.cur = res.next().0.get();
//                Some(res)
//            }
//            None => None,
//        }
//    }
//}

impl<'a, T: ?Sized + ListNode<'a, T>> List<'a, T> {
    pub const fn new() -> List<'a, T> {
        List {
            head: ListLink(None),
        }
    }

//    pub fn head(&self) -> Option<&'a mut T> {
//        self.head
//    }

    pub fn push_head(&mut self, node: &'a mut T) {
        let previous_head = core::mem::replace(&mut self.head, ListLink(None));

        if let Some(node) = previous_head.0 {
            *(node.next()) = ListLink(Some(node));
       }
       self.head = ListLink(Some(node));
    }

//    pub fn push_tail(&self, node: &'a mut T) {
//        node.next().0.set(None);
//        match self.iter().last() {
//            Some(last) => last.next().0.set(Some(node)),
//            None => self.push_head(node),
//        }
//    }

    pub fn pop_head(&mut self) -> Option<&'a mut T> {
        let remove = self.head.0;
        match remove {
            Some(node) => self.head = *node.next(),
            None => self.head = ListLink(None),
        }
        remove
    }

//    pub fn iter(&self) -> ListIterator<'a, T> {
//        ListIterator {
//            cur: self.head.0.get(),
//        }
//    }
}
*/
