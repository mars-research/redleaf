...
pub fn trusted_entry(s: Box<dyn syscalls::Syscall + Send + Sync>,
    ...
    #[cfg(not(any(feature = "benchnet", feature = "benchnvme")))]
    {
        ...
        rv6.sys_spawn_domain(rv6.clone_rv6().unwrap(), RRefVec::from_slice("/init".as_bytes()), RRefVec::from_slice("/init".as_bytes()), array_init::array_init(|_| None)).unwrap().unwrap();
    }
}
