...
macro_rules! generate_tpm {
    ($($(#[$attr:meta])* fn $func:ident(&self $(,)? $($arg:ident : $ty:ty),* $(,)? ) $(-> $ret:ty)? );* $(;)?) => {
        ...
        impl ::interface::tpm::UsrTpm for UsrTpm {
            fn clone_usrtpm(&self) -> RpcResult<alloc::boxed::Box<dyn ::interface::tpm::UsrTpm>> {
                Ok(box Self::new(self.tpm.clone_tpmdev()))
            }

            $(
                $(#[$attr])*
                fn $func(&self, $($arg: $ty,)*) $(-> $ret)? {
                    Ok(::libtpm::$func(&*self.tpm, $($arg), *))
                }
            )*
        }
    };
...
generate_tpm!(
        ...
        fn tpm_validate_locality(&self, locality: u32) -> RpcResult<bool>;
        ...
        fn relinquish_locality(&self, locality: u32) -> RpcResult<bool>;
    
        fn tpm_deactivate_all_localities(&self) -> RpcResult<bool>;
    
        /// Requests the TPM to switch to the locality we choose and wait for TPM to acknowledge our
        /// request
        fn tpm_request_locality(&self, locality: u32) -> RpcResult<bool>;
    
        /// Reads the TPM ID from device register
        fn read_tpm_id(&self, locality: u32) -> RpcResult<()>;
        ...
        fn tpm_get_burst(&self, locality: u32) -> RpcResult<u16>;
    
        /// Busy-wait in a loop for a particular status flag to be set
        fn wait_for_status_flag(&self, locality: u32, flag: u8, timeout_ms: usize) -> RpcResult<bool>;
    
        /// Writes data to the TPM FIFO.
        /// Here, `data.len < burst_count`
        fn tpm_write_data(&self, locality: u32, data: &[u8]) -> RpcResult<usize>;
    
        /// Checks TPM status register to see if there is any data available
        fn is_data_available(&self, locality: u32) -> RpcResult<bool>;
    
        /// Read data from TPM
        /// * Wait for data to be available
        /// * Receive as much as burst_count
        fn tpm_read_data(&self, locality: u32, data: &mut [u8]) -> RpcResult<usize>;
        ...
        fn tpm_recv_data(&self, locality: u32, buf: &mut Vec<u8>, rc: &mut u32) -> RpcResult<usize>;
    
        /// Wrapper for `tpm_write_data`
        /// This function waits for TPM to be in a state to accept commands before writing data to FIFO.
        fn tpm_send_data(&self, locality: u32, buf: &mut Vec<u8>) -> RpcResult<usize>;
        ...
        fn tpm_transmit_cmd(&self, locality: u32, buf: &mut Vec<u8>) -> RpcResult<()>;
        ...
        fn tpm_get_random(&self, locality: u32, num_octets: usize) -> RpcResult<bool>;
        ...
        fn tpm_init_bank_info(&self, locality: u32, hash_alg: u16) -> RpcResult<TpmBankInfo>;
    
        /// Table 3:208 - TPM2_PCR_GetCapability Command
        /// Obtain the banks that are allocated in TPM
        /// TODO: Return true/false, not structure
        fn tpm_get_pcr_allocation(&self, locality: u32) -> RpcResult<TpmDevInfo>;
        ...
        fn tpm_hash_sequence_start(&self, locality: u32, hash: TpmAlgorithms, object: &mut u32) -> RpcResult<bool>;
    
        /// Table 3:80 - TPM2_SequenceUpdate
        /// Update hash calculation in TPM
        fn tpm_sequence_update(&self, locality: u32, object: u32, buffer: Vec<u8>) -> RpcResult<bool>;
        ...
        fn tpm_flush_context(&self, locality: u32, flush_handle: u32) -> RpcResult<bool>;
);
