...
impl SetResponse {
    fn new(req_id: u32) -> Self {
        SetResponse {
            ...
            stored: [b'S', b'T', b'O', b'R', b'E', b'D'],
            line_end: [b'\r', b'\n'],
        }
    }
}
...
impl Decoder {
    ...
    pub fn decode_as_struct(&self) -> Result<ClientValue, DecodeError> {
        // The frame header is 8 bytes long, as follows (all values are 16-bit integers
        //     in network byte order, high byte first):
        //
        // 0-1 Request ID
        ...
        let req_hdr: &'static RequestHeader = unsafe {
            &*(self.reader.as_ptr() as *mut u8 as *mut RequestHeader)
        };

        let op = req_hdr.mode_str;

        trace!("op {:?}", op);
        // parse opcode
        match &op {
            b"set " => {
                let set_req: &'static SetRequest = unsafe {
                    &*(self.reader.as_ptr().offset(mem::size_of::<RequestHeader>() as isize) as *mut u8 as *mut SetRequest)
                };

                /*unsafe {
                    for i in 0..self.reader.len() {
                        print!("{:02x} ", self.reader[i]);
                        if (i + 1) % 16 == 0 {
                            print!("\n");
                        }
                    }
                    println!("");
                    let k = String::from_raw_parts(&set_req.key as *const _ as *mut u8, 8, 8);
                    let v = String::from_raw_parts(&set_req.value as *const _ as *mut u8, 8, 8);
                    println!("setreq: key : {}, value : {}", k, v);
                    core::mem::forget(k);
                    core::mem::forget(v);
                };*/
