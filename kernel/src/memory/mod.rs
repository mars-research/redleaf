...
use crate::memory::buddy::{BuddyFrameAllocator, BUDDY};
use core::alloc::{GlobalAlloc, Layout};
use core::mem::transmute;
use core::ptr::NonNull;
use log::trace;
use slabmalloc::{AllocationError, Allocator, ZoneAllocator};
use spin::Mutex;

pub use self::buddy::BuddyFrameAllocator as PhysicalMemoryAllocator;
pub use crate::arch::memory::{
    kernel_vaddr_to_paddr, paddr_to_kernel_vaddr, PAddr, VAddr, BASE_PAGE_SIZE,
};
                ...
                match zone_allocator.allocate(layout) {
                    ...
                    Err(AllocationError::OutOfMemory) => {
                        ...
                        if layout.size() <= ZoneAllocator::MAX_BASE_ALLOC_SIZE {
                            ...
                            zone_allocator
                                .refill(layout, transmute(vframe as usize))
                                .expect("Failed to refill ZoneAllocator");
                        } else {
                            let huge_layout =
                                layout.align_to(2 * 1024 * 1024).expect("Could not align?");
                            ...
                            let vframe: *mut u8 = frame.kernel_vaddr().as_mut_ptr();
                            zone_allocator
                                .refill_large(layout, transmute(vframe as usize))
...
fn text_end() -> u64 {
    ...
    unsafe { &__text_end as *const _ as u64 }
}
...
fn rodata_start() -> u64 {
    ...
    unsafe { &__rodata_start as *const _ as u64 }
}
...
fn rodata_end() -> u64 {
    ...
    unsafe { &__rodata_end as *const _ as u64 }
}
...
fn data_start() -> u64 {
    ...
    unsafe { &__data_start as *const _ as u64 }
}
...
fn data_end() -> u64 {
    ...
    unsafe { &__data_end as *const _ as u64 }
}
...
fn bss_start() -> u64 {
    ...
    unsafe { &__bss_start as *const _ as u64 }
}
...
fn bss_end() -> u64 {
    ...
    unsafe { &__bss_end as *const _ as u64 }
}
...
fn tdata_start() -> u64 {
    ...
    unsafe { &__tdata_start as *const _ as u64 }
}
...
fn tdata_end() -> u64 {
    ...
    unsafe { &__tdata_end as *const _ as u64 }
}
